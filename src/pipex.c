...
/**/
/* //a renommer ? */
/* int from_input_to_output(char *file, int fd[2], t_cmd cmd, char **envp,
	int order) */
/* { */
/*   int infile; */
/*   int outfile; */
...
int	add_first_node(char **av, char **envp, int fd[2], t_data **data)
{
	...
	new_node->file = av[1];
	new_node->fd_input = -1;
	new_node->fd_output = fd[1]; // fd[1] for the first node
...
int	add_last_node(char **av, char **envp, int fd[2], t_data **data)
{
	...
	tmp->next = new_node;
	new_node->file = av[4];
	new_node->fd_input = fd[0]; // fd[0] for the last node
...
int	init(char **av, char **envp, int fd[2], t_data **data)
{
	if (pipe(fd) == -1)
	{
...
int	main(int ac, char **av, char **envp)
{
  ...
  data = NULL;
	...
	/*   fork_error(fd, &data); */
	/* if (pid == 0) */
	/*   from_input_to_output(fd, &data, envp, 0); */
	/* pid = fork(); */
	/* if (pid == -1) */
	/* { */
	...
	/* } */
	/* if (pid == 0) */
	/*   from_pipe_to_file(fd, &data, envp, 2); */
	/* waitpid(pid, &status, 0); */
	/* exit_end(data, fd); */
}
